...
function getEc2CreateTime(instance: Instance): Date {
  ...
  return date ? date : (instance.LaunchTime ?? new Date());
}
  ...
  for (const reservation of output.Reservations ?? []) {
    for (const instance of reservation.Instances ?? []) {
      ...
      resources.push({
        ...
        startTime: instance.LaunchTime!.toISOString(), // TODO suspect with the !
        state,
        tags,
        tagsHash,
      });
...
async function listTagsForEcsResource(
  arn: string,
): Promise<EcsTag[] | undefined> {
