...
export class AutoState extends Construct {
  constructor(scope: Construct, id: string, props: AutoStateProps) {
    ...
    const deletedRdsInstance = new CallAwsService(this, 'DeleteRdsInstance', {
      ...
      parameters: {
        DbInstanceIdentifier: JsonPath.stringAt('$.resource.id'),
        SkipFinalSnapshot: JsonPath.stringAt(
          '$.resource.tags.skipFinalSnapshot'
        ),
        FinalDBSnapshotIdentifier: JsonPath.stringAt(
          '$.resource.tags.finalSnapshotIdentifier'
        ),
      },
    ...
    const deleteRdsClusterInstance = new CallAwsService(
      this,
      'DeleteRdsClusterInstance',
      {
        service: 'rds',
        action: 'deleteDBInstance',
        parameters: {
    ...
    const deleteRdsClusterInstances = new SfnMap(this, 'DeleteRdsInstances', {
      ...
      .iterator(deleteRdsClusterInstance)
      .next(deleteRdsCluster);
    ...
    const routeAction = new Choice(this, 'ActionRouter')
      ...
      .when(
        Condition.and(
          Condition.booleanEquals('$.execute', true),
          Condition.stringEquals('$.resource.type', 'ec2-instance'),
          Condition.stringEquals('$.action', 'reboot')
        ),
        ec2Reboot
      )
      .when(
    ...
    eventRouter.when(
      ...
      wait.next(invokeScheduler).next(routeAction)
    );
    eventRouter.when(
